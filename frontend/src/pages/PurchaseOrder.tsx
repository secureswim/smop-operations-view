...
const PurchaseOrder = () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowModal(false);
      setForm({ supplierId: "", items: [{ materialId: "", quantity: "", unitPrice: "" }] });
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                ) : pos.map((o: Record<string, unknown>) => (
                  <tr key={o.id as string} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                    <td className="px-4 py-3 font-mono text-foreground text-xs">{(o.poNumber || o.id) as string}</td>
                    <td className="px-4 py-3 text-foreground">{((o.supplier as Record<string, unknown>)?.name ?? o.supplierId) as string}</td>
                    <td className="px-4 py-3 text-foreground">{Array.isArray(o.items) ? o.items.length : 0} items</td>
                    ...
                    <td className="px-4 py-3">
                      {o.status === "DRAFT" && (
                        <button onClick={() => statusMutation.mutate({ id: o.id as string, status: "PENDING_APPROVAL" })}
                          className="px-2.5 py-1 rounded-md bg-primary/10 text-primary text-xs font-medium hover:bg-primary/20 transition-colors">Submit</button>
                      )}
                      {o.status === "PENDING_APPROVAL" && (
                        <button onClick={() => statusMutation.mutate({ id: o.id as string, status: "APPROVED" })}
                          className="px-2.5 py-1 rounded-md bg-success/10 text-success text-xs font-medium hover:bg-success/20 transition-colors">Approve</button>
                      )}
                    </td>
                  </tr>
                ))}
